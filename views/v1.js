Q('View',{
                ...
                },
                {
                    ...
                    listener:{
                        ...
                        move:function(){
                            if (!Q.isViewMOve) {
                                // x轴方向上滑动切换场景
                                if (Q.event.delay.x >20) {
                                    var cont = Q('cont'),v = cont.getByName('v5'), _this = Q('ibg').getByName('i5');
                                    
                                    if(cont.aty !== v){
                                        Q.qie(cont.aty,v,_this);
                                    }
                                } else if (Q.event.delay.x < -20) {
                                    var cont = Q('cont'),v = cont.getByName('v2'), _this = Q('ibg').getByName('i2');
                                ...
                                if (Math.abs(Q.event.delay.x) >20) Q.isViewMOve = true;
                    ...
                    listener:{
                        ...
                        move:function() {
                            if (!Q.isViewMOve) {
                                // x轴方向上滑动切换场景
                                if (Q.event.delay.x >20) {
                                    var cont = Q('cont'),v = cont.getByName('v1'), _this = Q('ibg').getByName('i1');
                                    
                                    if(cont.aty !== v){
                                        Q.qie(cont.aty,v,_this);
                                    }
                                } else if (Q.event.delay.x < -20) {
                                    var cont = Q('cont'),v = cont.getByName('v3'), _this = Q('ibg').getByName('i3');
                        ...
                        move:function() {
                            if (!Q.isViewMOve) {
                                ...
                                if (Q.event.delay.x >20) {
                                    ...
                                    var cont = Q('cont'),v = cont.getByName('v4'), _this = Q('ibg').getByName('i4');
    ...
    // 设置每一屏的垂直距离可移动的距离
            ...
            home.minY = minY(bound)
            home.width = '100w';
            home.height = viewHeight(bound);

            var home2 = con.getByName('v2');
            var bound2 = home2.getBounds();
            Q.layout.vertical(home2,home2,{
                space:0,
            });
            home2.maxY = 0;//Q('head').height;
            home2.minY =i1.y - bound2.height-Q('head').height;
            home2.width = '100w';
            home2.height = viewHeight(bound2);

            var home3 = con.getByName('v3');
            var bound3 = home3.getBounds();
            Q.layout.vertical(home3,home3,{
                space:0,
            });
            home3.width = '100w';
            home3.height = viewHeight(bound3);
            home3.maxY = 0;
            home3.minY = minY(bound3);

            var home4 = con.getByName('v4');
            var bound4 = home4.getBounds();
            
            home4.width = '100w';
            home4.height = viewHeight(bound4);
            home4.maxY = 0;
            home4.minY = minY(bound4);
            ...
            function viewHeight (view) {
                var minHeight = Q.app.height - Q('head').height - Q('ibg').height;
                return  height = view.height > minHeight ? view.height: minHeight;

            }
            // 设置minY
            function minY (isBound) {
                var minY = i1.y - isBound['height'] - Q('head').height;
                return  minY = minY > 0 ? 0 : minY;
            }
            ...
            Q.qie = function(a,b,c){
                ...
                function changeView (num) {
                    // 切换方式的方法
                    if ( (a == view[view.length-1] && b == view[1]) || (a == view[1] && b == view[view.length-1]) ) {
                        num = -num
                    }
            ...
            this.run = function(){
                ...
                vy =(Q.hua.mudi -Q.hua.y)*.2*Q.event.rate;
                Q.hua.y += vy;
                // console.log(vy)
                if(Math.abs(vy)<1)vy=0;
            }

             for (var i = 2, len = view.length; i < len; i++) {
                view[i].x = Q.app.width;
                view[i]
            }
